  ...
  loop %winList%
  {
    ...
    loop %count%
    {
      { ; Get the name of this menu item.
        name := ""
        ; Get size needed.
        size++ := DllCall("GetMenuString",  "Uint",hMenu,  "Uint",A_Index-1
                          , "Uint",0,  "Int",0,  "Uint",0x400)
        size *= 2  ; Make room for UNICODE strings.
        VarSetCapacity(name, size)
        ; Get menu text.
        DllCall("GetMenuString",  "Uint",hMenu,  "Uint",A_Index-1
                , "Str",name,  "Int",size,  "Uint",0x400)
      ...
      {
        isChecked := DllCall("GetMenuState",  "Uint",hMenu,  "Uint",A_Index-1
                              , "Uint",0x400,  "Uint") & 0x8
